...
class GlobalModel:
    ...
    '''
    Find the solution of the problem: the root of Eq_sys(). It starts with the initial solution init, and it varies its energy until it finds the root. If the problem dont converge with this algorithm this could be changed to a better one. This algorithm has problems when the density  is near the critical density, it is to high, or the power density is too high. 
    Args:
        -pwr: Power transmited to the plasma chamber (W/m^3).
