...
class GlobalModel:
    ...
    '''
    def __init__(self,reactions,variable_specie,L=10,R=3.1,gamma=0.1,Ti=0.2,Tg=300):
    ...
    def __Recombination_rate(self,e,n0):
        ...
        tau_rec=2*(2-self.gamma)*self.V/self.A_chamber/self.gamma/vth+A0_2/Dh
        return 1/tau_rec
    '''
    ...
    def __Specie_balance(self,concentrations,e):
        ...
        for reaction in self.surface_reactions:
                ...
                if reac in self.ions:
                    S[i,i]-=nr*concentrations[i]*self.__Bohm(e,reac[0])*self.__Aeff_V(n0)
                ...
                for l in range(len(reaction[3])):
                    ...
                    k=self.species_list.index(prod)
                    if reac in self.ions:
                        S[i,k]+=n*concentrations[i]*self.__Bohm(e,reac[0])*self.__Aeff_V(n0)
                    if reac in self.neutrals:
                        S[i,k]=n*concentrations[i]*self.__Recombination_rate(e,n0)
        for k in range(len(self.species_list)):
            equations[k]+=sum(S[:,k])
        #Delete the equations of the electron and the variable specie.
        var_index=self.species_list.index(self.variable_specie)
